...
public class TestTaskCalendar {
    ...
    public static void main(String[] args) {
        ...
        for (Taskmodel t : service.getSortedTasks()) {
        ...
        /* === 2. 取得排序後清單 === */
        List<Taskmodel> sorted = service.getSortedTasks();
        System.out.println("📋 依 截止日→優先序 排序後：");
        for (Taskmodel t : sorted) {
            System.out.printf("  %s | 截止: %s | 優先: %s%n",
                    t.getTitle(), t.getDueDate(), t.getPriority());
        }

        /* === 3. 取得近 7 天任務 === */
        List<Taskmodel> next7 = service.getTasksForNext7Days();
        System.out.println("\n📆 近 7 天內任務：");
        for (Taskmodel t : next7) {
            System.out.printf("  %s | 截止: %s | 優先: %s%n",
                    t.getTitle(), t.getDueDate(), t.getPriority());
        }
